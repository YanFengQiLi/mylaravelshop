...
class CreateGroupGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('group_goods', function (Blueprint $table) {
            ...
            $table->string('thumb')->comment('商品封面图');
            $table->text('images')->comment('商品轮播图');
            ...
            $table->unsignedInteger('open_group_number')->comment('已开团数量');
            $table->dateTime('end_time')->comment('拼团截止日期');
            $table->boolean('is_auto')->comment('自动成团');
